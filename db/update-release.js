...
import { formatISODate } from "@/components/releases/utils/date-utils";
 ...
 */
function makeJsonpPutRequest(endpoint, releaseId, data) {
 ...
 */
export async function updateRelease(releaseId, releaseData) {
    try {
        console.log(`Updating release ${releaseId}:`, releaseData);
        const result = await makeJsonpPutRequest('/api/releases', releaseId, releaseData);
        if (!result.success) {
 ...
 */
export function transformToBackendFormatForUpdate(formData) {
    const transformed = {};
    const fieldMappings = {
        ...
        'Date_delivered_by_vendor': formatISODate(formData.deliveredDate || ''),
        ...
        'Notification_date_for_deployment_to_test': formatISODate(formData.tdNoticeDate || ''),
        'Date_deployed_to_test': formatISODate(formData.testDeployDate || ''),
        'Date_of_test_commencement': formatISODate(formData.testStartDate || ''),
        'Date_of_test_completion': formatISODate(formData.testEndDate || ''),
        'Date_deployed_in_production': formatISODate(formData.prodDeployDate || ''),
 ...
 */
export async function updateReleaseFromForm(releaseId, formData) {
