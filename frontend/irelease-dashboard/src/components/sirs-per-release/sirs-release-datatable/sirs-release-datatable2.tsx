...
import { ChevronDown, ChevronLeft, ChevronRight, Calendar, MoreVertical } from "lucide-react"
...
const saveDateRangeDetails = (startDate: string, endDate: string) => {
  try {
    ...
    console.warn('Failed to save date range details from localStorage:', error);
  }
};
...
interface SirReleaseDataTableProps {
  // Props passed from parent component (where SirsReleaseFilters is used)
  selectedRelease?: string;
  selectedIteration?: string;
  globalFilter?: string;
  columnVisibility?: Record<string, boolean>;
  onExportCSV?: (data: any[], visibleColumns: any[]) => void;
  onExportExcel?: (data: any[], visibleColumns: any[]) => void;
  onExportJSON?: (data: any[], visibleColumns: any[]) => void;
}
...
export function SirReleaseDataTable({
  ...
  selectedIteration = "",
  globalFilter = "",
  columnVisibility = allColumns.reduce((acc, col) => ({ ...acc, [col.key]: true }), {}),
  onExportCSV,
  onExportExcel,
  onExportJSON
  ...
  const transformJsonData = () => {
    return sirReleaseData.map((item, index) => ({
      id: index + 1,
  ...
  const [data, setData] = useState(transformJsonData())
  ...
  // Filter data based on props from filters component AND date range
  const filteredData = data.filter(item => {
    // Filter by release version if selected
    const matchesRelease = !selectedRelease || item.release_version === selectedRelease;
    
    // Filter by iteration if selected
    const matchesIteration = !selectedIteration || item.iteration === selectedIteration;
    
    // Filter by global search if provided
    ...
    return matchesRelease && matchesIteration && matchesGlobalSearch && matchesDateRange
  })
  ...
  // Export functions - now pass data to parent component
  const handleExportCSV = () => {
    ...
    if (onExportCSV) {
      onExportCSV(dataToExport, visibleColumns);
    }
  }
  ...
  const handleExportExcel = () => {
    ...
    if (onExportExcel) {
      onExportExcel(dataToExport, visibleColumns);
    }
  }
  ...
  const handleExportJSON = () => {
    ...
    if (onExportJSON) {
      onExportJSON(dataToExport, visibleColumns);
    }
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      {/* REMOVED: The entire header section with filters (now handled by SirsReleaseFilters component) */}
      
      {/* Only show Date Range and Delete functionality here */}
      <div className="bg-gray-50 border-b border-gray-200 p-6">
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
          {/* Left Section - Empty since filters are at the top */}
          <div className="flex flex-col md:flex-row gap-3 xl:flex-1 xl:max-w-2xl">
            {/* This section intentionally left empty - filters are in SirsReleaseFilters */}
          </div>
        ...
        {/* Filter status */}
        {(selectedRelease || selectedIteration || globalFilter || dateRange) && (
          <div className="mt-2 text-sm text-gray-500">
            Showing {sortedAndFilteredData.length} SIRs
            {selectedRelease && ` for release: ${selectedRelease}`}
