...
import { loadColumnVisibility, saveColumnVisibility, loadDateRangeFilter, saveDateRangeFilter, loadDateRangeDetails, saveDateRangeDetails, clearDateRangeDetails, loadItemsPerPage, saveItemsPerPage } from "./utils/mo-storage-utils";
...
export function MoReleasesDataTable({ filteredData }) {
    ...
    const filteredDataInternal = data.filter(item => {
        ...
        const matchesGlobalSearch = !globalFilter ||
            Object.values(item).some(value => String(value).toLowerCase().includes(globalFilter.toLowerCase()));
        // Date range filter
    ...
    const handleAddRelease = useCallback((newRelease) => {
        setData(prev => [...prev, newRelease]);
        setAddDialogOpen(false);
        setCurrentPage(1);
        toast.success(`Successfully created new release ${newRelease.releaseId}`);
    }, []);
    const handleEditRelease = useCallback((updatedRelease) => {
        setData(prev => prev.map(item => item.id === updatedRelease.id ? updatedRelease : item));
    ...
    const handleExportSingleRelease = useCallback((release) => {
    ...
    const openDeleteDialog = useCallback((release) => {
        setReleaseToDelete(release);
        setDeleteDialogOpen(true);
    }, []);
    const openEditDialog = useCallback((release) => {
        setReleaseToEdit(release);
        setEditDialogOpen(true);
    }, []);
    // Show loading state (similar to releases-datatable.tsx)
    if (loading) {
        return (_jsx("div", { className: "flex-1 flex items-center justify-center", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto" }), _jsx("p", { className: "mt-4 text-gray-600", children: "Loading releases from database..." })] }) }));
    }
    return (_jsxs("div", { className: "flex-1 flex flex-col overflow-hidden", children: [_jsxs("div", { className: "flex-shrink-0 bg-white", children: [_jsx(ReleasesHeader, { selectedRowsCount: selectedRows.size, totalFilteredCount: sortedAndFilteredData.length, globalFilter: globalFilter, dateRange: dateRange }), _jsx(ReleasesFilters, { globalFilter: globalFilter, setGlobalFilter: setGlobalFilter, dateRange: dateRange, setDateRange: setDateRange, startDate: startDate, setStartDate: setStartDate, endDate: endDate, setEndDate: setEndDate, sortOrder: sortOrder, setSortOrder: setSortOrder, columnVisibility: columnVisibility, toggleColumnVisibility: toggleColumnVisibility, resetColumnVisibility: resetColumnVisibility, itemsPerPage: itemsPerPage, setItemsPerPage: setItemsPerPage, onAddRelease: () => setAddDialogOpen(true), onBulkDelete: openBulkDeleteDialog, selectedRowsCount: selectedRows.size, totalFilteredCount: sortedAndFilteredData.length, exportToCSV: handleExportCSV, exportToExcel: handleExportExcel, exportToJSON: handleExportJSON, onApplyDateRange: applyDateRange, onClearDateRange: clearDateRange })] }), _jsxs("div", { className: "flex-1 flex flex-col min-h-0 isolate", children: [_jsx("div", { className: "flex-1 overflow-hidden relative", children: _jsx(ReleasesTable, { data: paginatedData, visibleColumns: visibleColumns, selectedRows: selectedRows, onToggleRowSelection: toggleRowSelection, onToggleSelectAll: toggleSelectAllOnPage, onEditRelease: openEditDialog, onDeleteRelease: openDeleteDialog, onExportSingleRelease: handleExportSingleRelease }) }), _jsx(ReleasesPagination, { currentPage: currentPage, totalPages: totalPages, itemsPerPage: itemsPerPage, totalItems: sortedAndFilteredData.length, startIndex: startIndex, onPageChange: setCurrentPage, visibleColumnsCount: visibleColumns.length })] }), _jsx(AddReleaseDialog, { open: addDialogOpen, onOpenChange: setAddDialogOpen, onSave: handleAddRelease, existingData: data }), _jsx(EditReleaseDialog, { open: editDialogOpen, onOpenChange: setEditDialogOpen, release: releaseToEdit, onSave: handleEditRelease }), _jsx(DeleteDialogs, { bulkDeleteOpen: bulkDeleteDialogOpen, setBulkDeleteOpen: setBulkDeleteDialogOpen, singleDeleteOpen: deleteDialogOpen, setSingleDeleteOpen: setDeleteDialogOpen, releaseToDelete: releaseToDelete, selectedRowsCount: selectedRows.size, onBulkDelete: handleBulkDelete, onSingleDelete: handleSingleDelete })] }));
}
