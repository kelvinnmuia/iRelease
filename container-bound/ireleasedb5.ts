...
async function fetchSirsReleasesDirect(): Promise<any> {
...
// ============================================
// COMBINED FUNCTIONS (FIXED)
// ============================================

// FIX 1: Updated to check both tables
...
// FIX 2: Main initialization that checks BOTH tables
export async function initializeAllDatabases(): Promise<{
  ...
  try {
    // Initialize both databases in parallel - each checks its own table
...
// FIX 3: Main sync function that syncs BOTH APIs
...
export async function getCombinedDashboardStats() {
  const [releasesStats, sirsReleases] = await Promise.all([
    getDashboardStats(),
    getAllSirsReleases()
  ]);
  
  const totalSirsReleases = sirsReleases.length;
  const closed = sirsReleases.filter(s => s.Bug_status === "CLOSED").length;
  const open = sirsReleases.filter(s => s.Bug_status === "OPEN").length;
  const inProgress = sirsReleases.filter(s => s.Bug_status === "IN_PROGRESS").length;
  
  return {
    releases: releasesStats,
    sirsReleases: {
      total: totalSirsReleases,
      closed,
      open,
      inProgress
    },
    combined: {
      ...
      totalSirsReleases: totalSirsReleases
    }
  };
}